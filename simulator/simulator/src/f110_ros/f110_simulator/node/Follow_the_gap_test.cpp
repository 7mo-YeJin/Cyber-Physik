...
float view_ang;
float largest_dist,largest_direction,smallst_direction, smallst_dist=50, gain, speed=1.0;
...
void callback_scan(const sensor_msgs::LaserScan::ConstPtr& msg) {
	...
	// ROS_INFO("length is-----%d-------/n",length);
	 ...
	 for (loop=floor(1.0/4*length)+1;loop<=floor(3.0/4*length)+1;loop++){
		...
		max_loop=loop;
		if (max_loop>=floor(17.0/36*length)+1 & max_loop<=floor(19.0/36*length)+1 & largest_dist > 5.5 ) { // 5.2 if the largest dist between -10 degree und 10 degree, high speed, 7.0 enough distance for bremsen
		speed=6.5; // 7					
		} else if(max_loop>=floor(7.0/16*length)+1 & max_loop<floor(17.0/36*length)+1 & max_loop>=floor(19.0/36*length)+1 & max_loop<=floor(9.0/16*length)+1 )  // -15 to 15 
		  { 
	    speed=5.0;
		  }else
		speed=1.3; // 1.33
		...
		if (smallst_dist<1.0) {  // 0.8
			adjust_ang=true;
			// -----ROS_INFO("---------!!!!adjust angle!!!!---------");
			gain=0.1/smallst_dist; // 1.0 too big, wanna smoothly
			alpha_min=-gain*(min_loop-floor(1.0/2*length)); // direction can be changed
	        smallst_direction=msg->angle_increment*alpha_min;
	        angle=smallst_direction;
	        // reset
	        largest_dist=0; 
	        max_loop=0;
	        min_loop=0;
	        smallst_dist=50; 
	        // ------ROS_INFO("smallst direction is     %f-------/n",smallst_direction);
		} else{
...
int main(int argc, char *argv[]) {
  ...
  ros::Publisher command_pub = n.advertise<ackermann_msgs::AckermannDriveStamped>("Test_drive", 1);
